...
use crate::types::pkcs11_type_impl;
...
pub(crate) fn pkcs11_attribute_type_impl(input: TokenStream) -> TokenStream {
    ...
    // Type impl
    let attribute_type_arms = name_pairs.iter().map(|v| {
        let (attr_name, const_name) = (&v.attr_name, &v.attr_type_const_name);
        quote! {
            #type_name::#attr_name(_) => #attr_type_name::#const_name,
        }
    });

    let inner_value_arms = name_pairs.iter().map(|v| {
        let attr_name = &v.attr_name;
        quote! {
    ...
    // From
    let from_ts = quote! {impl ::std::convert::From<&#type_name> for CK_ATTRIBUTE {
    ...
    // TryFrom
    let try_from_arms = name_pairs.iter().map(|v| {
        let (attr_name, const_name, ty) = (&v.attr_name, &v.attr_type_const_name, v.ty);
        quote! {
